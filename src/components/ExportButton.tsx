...
const ExportButtonComponent = ({ content, title, filename }: ExportButtonProps) => {
  ...
  return (
    <Popover.Root>
      <Popover.Trigger
        className="p-2 theme-text-secondary hover:theme-text hover:theme-active-bg theme-radius-base transition-colors duration-200"
      ...
      <Popover.Portal>
        <Popover.Positioner
          ...
          align="end"
          sideOffset={8}>
          <Popover.Popup className="w-48 md:w-64 lg:w-96 theme-bg theme-border-medium shadow-lg z-[9999] py-1 origin-[var(--transform-origin)] transition-[transform,scale,opacity] data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[starting-style]:scale-90 data-[starting-style]:opacity-0">
            {/* PDF Export */}
            <button
              onClick={exportAsPDF}
